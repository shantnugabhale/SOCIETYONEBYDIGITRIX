...
class BalanceSheetDisplayScreen extends StatefulWidget {
  final int? year;

  const BalanceSheetDisplayScreen({
    super.key,
    this.year,
  });

  @override
  State<BalanceSheetDisplayScreen> createState() => _BalanceSheetDisplayScreenState();
}

class _BalanceSheetDisplayScreenState extends State<BalanceSheetDisplayScreen> {
  ...
  void _editBalanceSheet() {
    if (_selectedBalanceSheet == null) return;
    
    Get.toNamed(
      '/balance-sheet-entry',
      arguments: {
        'year': _selectedBalanceSheet!.year,
        'balanceSheet': _selectedBalanceSheet,
      },
    )?.then((_) {
      _loadBalanceSheets();
      // Refresh the selected balance sheet
      if (_selectedBalanceSheet != null) {
        _firestoreService.getBalanceSheetByYear(_selectedBalanceSheet!.year).then((updated) {
          if (updated != null) {
            setState(() {
              _selectedBalanceSheet = updated;
            });
          }
        });
      }
  ...
  void _createNewBalanceSheet() {
    Get.toNamed('/balance-sheet-entry')?.then((_) => _loadBalanceSheets());
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: _isViewMode
            ...
            : null,
        actions: [
          if (_isViewMode && _selectedBalanceSheet != null)
            IconButton(
              icon: const Icon(Icons.edit),
              onPressed: _editBalanceSheet,
              tooltip: 'Edit',
            ),
          if (!_isViewMode)
            IconButton(
              icon: const Icon(Icons.add),
              onPressed: _createNewBalanceSheet,
              tooltip: 'Create New',
            ),
        ],
      ),
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              onPressed: _createNewBalanceSheet,
              icon: const Icon(Icons.add),
              label: const Text('Create Balance Sheet'),
